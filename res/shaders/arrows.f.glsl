...
vec4 arrowTileCenterCoord(vec4 pos) {
        return vec4((floor(pos.xy / ARROW_TILE_SIZE) + 0.5) * ARROW_TILE_SIZE, pos.zw);
}
...
vec2 integrate(vec2 here, vec2 p1, vec2 p2)
{
            vec2 diff = p2 - p1;
...
void main()
{
    float arrow_k = (1.0 - arrow(gl_FragCoord.xy, field(arrowTileCenterCoord(gl_FragCoord)) * ARROW_TILE_SIZE * 0.4));
    frag_color = arrow_k * vec4(normalize(field(gl_FragCoord)) * 0.5 + 0.5, 0.5, 1.0);
}
